...
class Trainer:
    ...
    def play(self, policy_path: str) -> None:
        ...
        self._prepare_play_env(env)
        # A trained SmoothSkate policy expects the settled actuator filter. Do
        # not replay the training warm-up ramp in the viewer, which otherwise
        # makes the first seconds look artificially choppy.
        if hasattr(env, "_smooth_filter_steps") and hasattr(env.cfg.control, "action_filter_warmup_steps"):
            env._smooth_filter_steps = env.cfg.control.action_filter_warmup_steps
    ...
    def _prepare_play_env(self, env) -> None:
        """Align playback commands with the policy's trained curriculum stage."""
        if hasattr(env, "set_training_iteration"):
            # Roller policies are trained for the final command distribution;
            # starting playback at stage zero silently asks them to stand still.
            env.set_training_iteration(1000, 24)
        if hasattr(env, "_smooth_filter_steps") and hasattr(env.cfg.control, "action_filter_warmup_steps"):
            env._smooth_filter_steps = env.cfg.control.action_filter_warmup_steps
